...
- **新设备开荒（当前版本）**：先"恢复"一台老设备的备份 → 重启 → 再开增量同步。恢复只带数据，**设备 ID 仍是本机自己的**（`sync-state.json` 不进备份包）
...
- **元数据首次全量引导未实现**：触发器建立（2026-07-22）之前的存量书永远进不了 changeset，全新设备靠增量同步收不到存量书；云端修剪会把旧 changeset 删掉，缺口静默永久化。**当前绕过方法：新设备先"恢复"备份再开增量同步**
...
| 现象 | 原因 | 处置 |
|---|---|---|
| 新设备书架空 | 存量书不在 changeset（已知缺口） | 先"恢复"备份开荒 |
