...
- **推送：事件驱动**——本地有未推变更（`_sync_log` 水位检查，纯本地零网络）后 **25 秒防抖**即推一轮，不受频率设置影响；"有变更 30 秒内上云、无变更零请求"
...
- **2a 元数据通道**（核心）✅ 2026-07-22 落地：迁移（_sync_log+触发器、book_status 增 position_changed_at/dwell_seconds）、打包/应用引擎、冲突矩阵（含 threads 消息级合并）、事件驱动推送 + 退出前推送 + 独立拉取周期、本地/云端日志修剪、安全快照、设置 UI、对话面板刷新按钮。`cargo test` 18/18。真机双设备验收：对话/星标/进度同步、逗留判定（乱翻不污染、挂机不抢戳）、防回环、墓碑均通过；已知遗留：同步落地后阅读器落点刷新曾有缺陷（reader-store 缓存 config 陈旧），已修，待最终确认
