...
## 进度快照（2026-07-22，额度耗尽前存档）

**已完成**：P0 对话管理、P1 导出+主题、P2 路线 A（MCP）、对话星标、回收站、L1 备份、L2 2a 元数据同步（含审计修复 8249186，cargo 21/21）、支线"刷新按钮增强"。PR #44/#45/#46 已开往上游。

**恢复后执行顺序**（按批，估算为 AI 结对工作日）：

1. **第一批·还债与快赢（约 1 天）**：真机双设备复验 L2 跳进度 → PR 4（L1 备份，机械操作 <1h）→ 支线小项：回收站警告文案（10min）、验证"多选按钮被顶掉"是否已过时（审计见 ListChecks 仍在）、失败通知颜色、新建标签勾选不入标签 BUG
2. **第二批·同步收尾（约 2 天）**：L2 2b 书籍通道（sha256 索引/上传/懒下载/云端未下载标识）+ l2-safety 快照回滚入口 + 2c 打磨（空闲调度、首次全量引导）
3. **第三批·支线中项（约 3 天）**：会话列表多选管理、通知体系静默化、对话右键菜单换 HTML、AI 对话网络搜索（优先 provider 自带搜索，免 key）、批量与智能标签（注意：一书多标签涉 schema 变更，要与同步表设计对齐）
4. **第四批·大件（各 2-3 天）**：Books_Converter 整合（PDF→EPUB→入库，复用辅助模型）、标签栏纵横切换、APP 帮助助手
5. **等作者**：PR 5（L2 同步）须 PR 1/3/4 合并后 rebase 再提；作者 3 个月无响应则 fork 转活跃维护版（README 顶部声明 + 发 release + 考虑改名）
6. **远期不承诺**：P2 路线 B（内嵌 MCP over HTTP）、L3 端到端加密、移动端
