...
> 本文档记录我们（fork 方）的定制计划，**不提交给上游**。最后更新：2026-07-24。
...
1. **第三批·支线中项（约 3 天）**：会话列表多选管理、对话右键菜单换 HTML、AI 对话网络搜索（优先 provider 自带搜索，免 key）、批量与智能标签（注意：一书多标签涉 schema 变更，要与同步表设计对齐）
2. **第四批·大件（各 2-3 天）**：Books_Converter 整合（PDF→EPUB→入库，复用辅助模型）、标签栏纵横切换、APP 帮助助手
